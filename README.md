...
standard for software development, we can easily adopt the framework for
software development to provide structure for any kind of research, ASC
projects included. GitHub is an online hosting service where each
project lives in its own repository maintained by me as the mentor.
After a project is completed, you can copy (i.e., *fork*) the repo. So
learning to use Git and GitHub not only aids collaboration, it will
...
line](https://happygitwithr.com/install-git#install-git) (you can
substitute R references with Python and RStudio with Positron) or
[downloading the latest source](https://git-scm.com/downloads). Then go
ahead and walk through the [Set up Git in VS
Code](https://code.visualstudio.com/docs/sourcecontrol/intro-to-git#_set-up-git-in-vs-code)
documentation, substituting VS Code with Positron.
...
files and folders that will be pushed to GitHub and those that are not.
...
repository on GitHub). We can have an ongoing conversation there about
each specific issue and close them out when a given issue is completed
or resolved. Be sure to tag collaborators you want to see a specific
comment (e.g., `@marcdotson`). Think of this as an email thread or Teams
channel except all of the conversations are in one place, easily
searchable, and automatically archived as part of the version control.
...
in your cloned repository without any impact on the centralized
repository hosted on GitHub. For example, I typically store PDFs of
papers and other related materials that I don’t want to (or can’t) share
in the `/private` folder so that resources I need area all within the
same directory on my computer.
...
> versions. Here’s two reasons:
>
> - Not all Python versions are (or were) backwards compatible (i.e., if
>   someone runs our code with a different version of Python, it might
>   not work).
...
> project environment is reproducible enables collaboration,
> future-proofing, and open science. Python is a big tent, and there are
> many ways to maintain a reproducible environment. I recommend using
...
> already been done as part of the
...
> version, open the terminal in Positron after opening the cloned
...
> version of Python to use for that project.
>
> This `.python-version` file is a flat text file, meaning it’s simple
> enough to be pushed to GitHub. When you open the cloned repository in
...
> libraries and library dependencies that we use. Note that this has
> already been *started* as part of the
> [asc-template](https://github.com/marcdotson/asc-template). The
> project library is stored in the `/.venv` folder, a hidden folder
> listed in the project organization. It isn’t a flat file and can get
> large, so it’s not something we push to GitHub. However, the
> `requirements.txt` file is what keeps track of the library versions
> and is a flat file that can be pushed to GitHub. To put it another
> way, we can use `requirements.txt` to recreate the needed project
> library in our cloned repository in the same way we could use
> `.python-version` to know what Python version to use. Simply open the
...
> library follwed by `pip install -r requirements.txt` to install the
...
exists in parallel and is focused on a specific task and often will be
tied to a specific issue. For example, we could have a branch called
`initial-model` and another branched called `data-cleaning`. For version
control to be useful, we need to be concise and descriptive with branch
and other naming conventions (i.e., no `final-final-draft-02` nonsense
here).
...
working in by looking at the bottom right corner in Positron. Assuming
...
to work in. Use the command palette and fine `Git: Checkout to...` to
...
data” or “Cleaned up errors to the final model” – something that will be
meaningful. It is these commit messages, together with the branch names,
that provide a record of the work we have done. With a descriptive
message, you are ready to
...
to GitHub. Similarly, to get changes others have pushed, we *pull* them
from GitHub.

<img src="figures/github_daily-work.png" style="width:90.0%"
data-fig-align="center" />

In Positron, this is often summarized as one step called *sync*, which
is pulling and then pushing in sequence. To summarize, the daily
workflow using GitHub looks like this:
...
- Pull changes that have been pushed to GitHub since you last worked on
  ...
  branch name in the bottom right corner).
- After working, stage and commit changes with a descriptive commit
  message.
...
what it sounds like – a request submitted to the repository admin to
pull the change you’ve made. This allows me to review what you’ve done,
have a conversation with you about it as part of the pull request itself
(which looks a lot like an issue tied to the branch specifically), and
eventually pull what you’ve done into `main`. I’ll then delete the
branch specific to that issue and close out the associated issue.

Note that when a branch is deleted on GitHub, it will still exist on
...
Make sure you’re working on the correct branch. Eventually you’ll want
to clean up branches that have been merged into main and closed on
GitHub by using the command palette and using `Git: Delete Branch...`
follwed by the branch name. You may also need to use `Git: Fetch` to
