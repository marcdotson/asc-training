...
- Once you've intalled packages, add them to the project library using `renv::snapshot()`.
- To install the specific packages of an already-existing project library, use `renv::restore()`.

Read [https://rstudio.github.io/renv/index.html](here) for more on using `{renv}`, including creating and maintaining Python environments.
