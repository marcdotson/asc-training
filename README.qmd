...
Similarly, while we can include a figure using LaTeX or Markdown, we can also use `knitr::include_graphics()`. Once again, the name of the code block is the label that can be referenced, for example `#| label: fig-elephant` or `{#fig-elephant}` referenced with `@fig-elephant` (note that the `fig-` prefix is required). 
